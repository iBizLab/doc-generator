<% if (item.getNoOrgIdEntities()) { %>\

### 启用组织权限范围，但未配置组织属性<sup class="footnote-symbol"> <font color=orange>[${item.getNoOrgIdEntities().size()}]</font></sup>
|    中文名  | 实体名   | 代码名      |  实体类型   |  存储模式 |  表名称  |  逻辑有效   |  联合主键   |  主状态   |  权限控制  |  启用审计    |  备注  |
| --------| --------   |------------| -----   |  --------|  --------|  --------|  -------- |  -------- | -------- | -------- |-------- |
<%  item.getNoOrgIdEntities().each{ entity -> %>\
