<% if (item.getDenyOPPrivMethods()) { %>\

### DENY权限请求接口<sup class="footnote-symbol"> <font color=orange>[${item.getDenyOPPrivMethods().size()}]</font></sup>
 | 请求路径| 请求方式| 实体   |    行为/集合    |
| -------- |-------- | --------|-------- |
<%  item.getDenyOPPrivMethods().each{ method ->
